...
## Notes
The interaction history (i.e. the list of previous question/answer pairs) is automatically handled by the client. You don't have to do anything about it.
If we want to retrieve the history, the function `sendInteraction("...")`, which returns type `Promise<IInteractionConsumerResponse>` will contain
the history under: `IInteractionConsumerResponse["history"]`.
